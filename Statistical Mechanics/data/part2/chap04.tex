...
\begin{remark}
	...
	% 它是$q,p$的函数，当存在外场时还是外场参量$y$的函数，但不是时间的显函数。
\end{remark}
...
% 而在量子理论中，
% 可测量物理量对应一个Hermite算符$\hat B$，其具有一系列本征值$B_s$和本征态$\ket s$：
% \[
% 	\hat B\ket s=B_s\ket s,\quad s=1,2,\ldots
% \]
% 系统所处的量子态为$\ket\psi$，经过一次测量，系统会坍缩到某一本征态$\ket s$上，分布函数就是系统坍缩到某一量子态的几率$\rho_s(t)\equiv\abs{\brkt s\psi}^2$，满足归一化条件
% \[
% 	\sum_s\rho_s(t)=\sum_s\brkt\psi s\brkt s\psi=\sum_s\brkt\psi\psi=1.
% \]
% 而测量的期望值为
% \begin{equation}
% 	\avg B(t)=\sum_s B_s\rho_s(t).
% \end{equation}
% 由不确定度关系，每个量子态在$\varGamma$空间中占据的相体积为$h^f$。
