...
\begin{theorem}{Coulomb定律}{Coulomb's law}
    真空中两个静止点电荷$q_1,q_2$分别位于$\bm r_1,\bm r_2$，$q_1$相对$q_2$的位置为$\bm r:=\bm r_1-\bm r_2$，则$q_1$受到$q_2$的静电力为：
    \begin{equation}
...
\begin{corollary}
    由Coulomb定律，位于$\bm r_1$的点电荷$q_1$在$\bm r$处激发的电场$\bm E_1$为
    \begin{equation}
        \label{eqn:E of q}
        \bm E_1=\frac{q_1}{4\pi\varepsilon_0}\frac{\bm r-\bm r_1}{|\bm r-\bm r_1|^3};
    \end{equation}
\end{corollary}
...
\begin{theorem}
    ...
    \begin{equation}
        \label{eqn:E(r) sum}
        \bm E=\sum_{i=1}^n\bm E_i=\frac1{4\pi\varepsilon_0}\sum_{i=1}^nq_i\frac{\bm r-\bm r_i}{|\bm r-\bm r_i|^3},
    \end{equation}
    如果电荷是连续(continuous)分布的，可以用电荷密度$\rho(\bm r)$描述，则求和变成积分：
    \begin{equation}
        \label{eqn:E(r)}
        \bm E(\bm r)=\frac1{4\pi\varepsilon_0}\int_V\rho(\bm r')\frac{\bm r-\bm r'}{|\bm r-\bm r'|^3}\d V'.
    \end{equation}
...
\begin{remark}
    若已知$\rho(\bm r)$，我们可以根据积分\eqref{eqn:E(r)}求解静电场分布，但是这并不总是最适合的方法，下面我们介绍Gauss定律。
\end{remark}
...
\begin{proof}
    ...
    \[
        \bm E\cdot\d\bm A=\frac q{4\pi\varepsilon_0}\frac{\cos\theta}{r^2}\d A=\frac q{4\pi\varepsilon_0}\d\Omega,
    \]
    其中$\d\bm A=\uvec n\d A$，$\uvec n$是曲面上该点向外的单位法向矢量，$\d A$是面积元；$r$是该点电荷到曲面上一点的距离，$\theta$是$\bm E$与$\uvec n$的夹角，$\d\Omega$是$\d A$对点电荷所张的立体角(solid angle)元。
...
\[
    \nabla\biggkh{\frac1{|\bm r-\bm r'|}}=-\frac{\bm r-\bm r'}{|\bm r-\bm r'|^3}.
\]
可将电场\eqref{eqn:E(r)}写成梯度的形式：
\[
    \bm E(\bm r)=-\frac1{4\pi\varepsilon_0}\nabla\biggkh{\int_V\frac{\rho(\bm r')}{|\bm r'-\bm r|}\d V'}
\]
\begin{definition}{标量势}{scalar potential}
    ...
    \begin{equation}
        \label{eqn:Phi-rho}
        \Phi(\bm r):=\frac1{4\pi\varepsilon_0}\int_V\frac{\rho(\bm r')}{|\bm r'-\bm r|}\d V'.
    \end{equation}
\end{definition}
...
\begin{example}{电偶层}{dipole layers}
    ...
    \[
        \Phi(\bm r)=\frac1{4\pi\varepsilon_0}\int_S\frac{\sigma(\bm r')}{|\bm r-\bm r'|}\d A'-\frac1{4\pi\varepsilon_0}\int_{S'}\frac{\sigma(\bm r')}{|\bm r-\bm r'+d\uvec n|}\d A',
    \]
    由三维Taylor展开，
    \[
        \frac1{|\bm r+\bm a|}=\frac1r+\bm a\cdot\nabla\biggkh{\frac1r}+\cdots,\quad a\ll r.
    \]
    可得当$d\to0$时(注意此处是对$\bm r'$求导)，
    \[
        \Phi(\bm r)\simeq\frac1{4\pi\varepsilon_0}\int_S\sigma(\bm r')d\uvec n\cdot\nabla'\biggkh{\frac1{|\bm r-\bm r'|}}\d A',
    \]
    其中
    \[
        \uvec n\cdot\nabla'\biggkh{\frac1{|\bm r-\bm r'|}}\d A'=-\frac{\cos\theta}{|\bm r-\bm r'|^2}\d A'=-\d\Omega.
    \]
    定义层矩(surface-dipole-moment density) 
    \[
        D(\bm r):=\lim_{d\to 0}\sigma(\bm r)d
    \]
    则
    \begin{equation}
...
\begin{equation}
    W=\frac1{4\pi\varepsilon_0}\sum_{i=1}^n\sum_{j=1}^{i-1}\frac{q_iq_j}{|\bm r_i-\bm r_j|}=\frac1{8\pi\varepsilon_0}\sum_{i\neq j}\frac{q_iq_j}{|\bm r_i-\bm r_j|}.
\end{equation}
对于连续分布的电荷：
\begin{equation*}
    \label{eqn:W-qq}
    W=\frac1{8\pi\varepsilon_0}\iint_V\frac{\rho(\bm r)\rho(\bm r')}{|\bm r-\bm r'|}\d V\nd V'.
\end{equation*}
...
\begin{example}
    ...
    平板电容器的电荷$Q=S\sigma$，由\exmref{exm:E of infinite plate} 可得电压$V=Ed=\sigma d/\varepsilon_0$，故电容为
    \begin{equation}
        C=\varepsilon_0\frac{S}{d},
    \end{equation}
    其中$S$为电容器的极板面积，$d$为极板间距。
\end{example}
...
利用Green第二公式\eqref{eqn:Green II}，我们令$\phi:=\Phi(\bm r'),\enspace\psi:=|\bm r-\bm r'|^{-1}$，则
\[
...
\begin{equation}
    \label{eqn:Phi-rho with boundary}
        \Phi(\bm r)=\frac1{4\pi\varepsilon_0}\int_V\frac{\rho(\bm r')}{|\bm r-\bm r'|}\d V'+\frac1{4\pi}\oint_{\p V}\biggfkh{\frac1{|\bm r-\bm r'|}\pv\Phi{n'}-\Phi(\bm r')\pp{n'}\biggkh{\frac1{|\bm r-\bm r'|}}}\d A',
\end{equation}
与式\eqref{eqn:Phi-rho}对比，上式多了两项边界$\p V$上的积分，分别等价于面电荷的电势($\p\Phi/\p n'$不连续)
\begin{subequations}
    \begin{align}
        \Phi(\bm r)&=\frac1{4\pi\varepsilon_0}\oint_{\p V}\frac{\sigma(\bm r')}{|\bm r-\bm r'|}\d A'\\
...
\begin{subequations}
    \begin{align}
        \Phi(\bm r)&=\frac1{4\pi\varepsilon_0}\oint_{\p V}D(\bm r')\pp{n'}\biggkh{\frac1{|\bm r-\bm r'|}}\d A'\\
...
\]
通常Neumann问题是在一个有限体积之外的，因此$\p V$除$V$的表面外还包括无穷远处，此时$\ave\Phi_{\p V}=0$。
...
Green函数可被拆解为
\[
    G(\bm r,\bm r')=\frac1{|\bm r-\bm r'|}+F(\bm r,\bm r'),
\]
...
采用直角网格划分，格点$(i,j)$处的电位为$\Phi_{i,j}$，网格间距为$h$，则
\begin{equation}
