...
\begin{theorem}{Gauss定律}{Gauss's law}
    电场$\bm E$在一个封闭曲面$\p V$上的通量(flux)为$V$内的所有电荷$q_i$求和：
    \begin{equation}
        \label{eqn:Gauss law}
        \oint_{\p V}\bm E\cdot\d\bm A=\frac q{\varepsilon_0}=\frac1{\varepsilon_0}\int_V\rho(\bm r)\d V,
    \end{equation}
    其中$q$是$V$内的总电荷量。
\end{theorem}
...
\begin{proof}
    由散度定理\eqref{eqn:div}以及Gauss定律\eqref{eqn:Gauss law}，可得
    \[
        \oint_{\p V}\bm E\cdot\d\bm A=\int_V\div\bm E\d V=\frac1{\varepsilon_0}\int_V\rho(\bm r)\d V\implies
        \int_V\biggkh{\div\bm E-\frac\rho{\varepsilon_0}}\d V=0.
    \]
    该式对任意$V$均成立。由此被积分式为零。
    式\eqref{eqn:divE}也被称为Gauss定律的微分形式。
\end{proof}
...
\begin{proof}
    ...
    \end{equation}
    由式\eqref{eqn:curlgrad}梯度的旋度为$\bm0$即证。
\end{proof}

\begin{corollary}
    由旋度定理\eqref{eqn:curl}，电场对任意闭合路径的环路积分为零：
    \begin{equation}
        \oint_{\p S}\bm E\cdot\d\bm\ell=\int_S(\curl\bm E)\cdot\d\bm A\equiv 0.
    \end{equation}
\end{corollary}
...
\end{equation}
与式\eqref{eqn:Phi-rho}对比，上式多了两项边界$\p V$上的积分，分别等价于：
\begin{itemize}
    \item 面电荷的电势($\p\Phi/\p n$不连续)
    \begin{subequations}
        \begin{align}
...
\begin{definition}{边界条件}{boundary condition}
    \begin{itemize}
        \item Dirichlet边界条件：给定表面上的电位
        \[
            \Phi|_{\p V}=f;
        \]
        \item Neumann边界条件：给定表面上电位的法向导数
        \[
            \edg{\pv\Phi n}_{\p V}=f;
        \]
        \item 混合边界条件：给定$\Gamma_{\text{D}}$上的电位和$\Gamma_{\text{N}}$上电位的法向导数
        \[
            \Phi|_{\Gamma_{\text{D}}}=f,\quad
            \edg{\pv\Phi n}_{\Gamma_{\text{N}}}=f',
        \]
        其中$\Gamma_{\text{D}},\Gamma_{\text{N}}$是表面$\p V$的一个划分，满足$\Gamma_{\text{D}}\cup\Gamma_{\text{N}}=\p V$且$\Gamma_{\text{D}}\cap\Gamma_{\text{N}}=\varnothing$；
        \item Robin边界条件：给定表面上电位与其法向导数的线性组合
        \[
            \edg{\alpha\Phi+\beta\pv\Phi n}_{\p V}=f.
        \]
        % 其中$\alpha,\beta$可以是函数。
    \end{itemize}
\end{definition}
...
\[
    \int_V(U\cancel{\lapla U}+\nabla U\cdot\nabla U)\d V=\oint_{\p V}\cancel{U\pv Un}\d A,
\]
即
\[
...
\begin{remark}
    对于Dirichlet边界条件(以及混合边界条件)，可以由边界确定$U\equiv 0$，故解是唯一的；
    对于Neumann边界条件，$U\equiv\const$，对电场而言$-\nabla\Phi$也是唯一的。
\end{remark}

\begin{remark}
    对于Robin边界条件，
    \[
        \edg{\alpha U+\beta\pv Un}_{\p V}=0\implies\int_V\abs{\nabla U}^2\d V=-\oint_{\p V}\alpha U^2/\beta\d A,
    \]
    当$\alpha,\beta$同号时，仍可得出$U\equiv 0$，解是唯一的。
    但当$\alpha,\beta$异号时，无法保证解的唯一性。
\end{remark}
...
\begin{theorem}
    ...
    \]
    如果$V$包括无穷远处，则\eqref{eqn:Neumann Green condition}变为$\p G/\p n'|_{\p V}\equiv 0$，此时$\ave\Phi_{\p V}=0$。
\end{theorem}
...
\begin{example}{Green函数举例}{}
    ...
    \]
    在上半空间$V=\set{(x,y,z)}{z>0}$中，对于Dirichlet边界条件，需要在边界处即$z=0$时$G(\bm r,\bm r')=0$。可取$\bm r'$的对称点$\bm r''=(x',y',-z')$，并取
    \begin{align*}
