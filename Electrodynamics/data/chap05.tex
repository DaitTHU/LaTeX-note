...
\begin{example}{真空中电磁波}{electromagnetic wave}
    ...
    \begin{align*}
        ...
        \lapla\bm B&=\nabla(\div\bm B)-\curl(\curl\bm B)=0-\mu_0\varepsilon_0\pp t(\curl\bm E)
...
\begin{definition}
    {标量势和矢量势}{scalar potential and vector potential}
    引入标量势$\Phi$和矢量势$\bm A$，使得电场和磁场可以表示为
    \begin{subequations}
        \begin{align}
...
\begin{corollary}
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    下一步是解耦合(decouple)式\eqref{eqn:Phi-A coupled}中的$\Phi$和$\bm A$。
\end{corollary}

\begin{theorem}
    {规范不变性}{gauge invariance}
    电场和磁场在规范变换(gauge transformation)中保持不变：
    \begin{equation}
...
\begin{proof}
    规范变换后的$\Phi',\bm A'$得到的电场和磁场为
    \begin{align*}
        \bm E'&=-\nabla\biggkh{\Phi-\pv\varLambda t}-\pp t(\bm A+\nabla\varLambda)=-\nabla\Phi-\pv{\bm A}t=\bm E;\\
        \bm B'&=\curl(\bm A+\nabla\varLambda)=\curl\bm A=\bm B.
        \qedhere
    \end{align*}
\end{proof}

\begin{corollary}
    总能将任意$(\Phi,\bm A)$规范变换为满足特定约束条件的$(\Phi',\bm A')$。
\end{corollary}
...
\[
    \Phi(\bm r,t)=\frac1{4\pi\varepsilon_0}\int_V\frac{\rho(\bm r',t)}{\abs{\bm r-\bm r'}}\d V'
 \]
这说明：标量势$\Phi(\bm r,t)$是由电荷密度$\rho(\bm r',t)$产生的瞬时Coulomb势\footnote{这是其被称为Coulomb规范的原因。}，%二者在不同地方同时改变，
在空间中的传播没有任何延迟。
...
\begin{example}{Coulomb规范下电磁波的表达}{em wave in Coulomb gauge}
    ...
    \[
        \lapla\bm A-\frac1{c^2}\pv[2]{\bm A}t=\bm 0\implies\bm A=\bm A_0\e{\i(\bm k\cdot\bm r-\omega t)}.
    \]
    从而
    \begin{align*}
...
\begin{equation}
    ...
    \lapla\Psi-\frac1{c^2}\pv[2]\Psi t=-4\pi f(\bm r,t).
\end{equation}
%其Green函数
%\biggkh{\lapla-\frac1{c^2}\pp[2]t}G(\bm r,t;\bm r',t')=-4\pi\vd(\bm r-\bm r')\vd(t-t'),
%表示在$\bm r'$处的点源，且仅在$t'$时刻激发。
对$t$进行Fourier变换，%频域里
\[
    (\nabla^2+k^2)\hat\Psi(\bm r,\omega)=-4\pi\hat f(\bm r,\omega),
\]
其中$k:=\omega/c$。
则频域中的Green函数$\hat G_k(\bm r,\bm r')$满足
\[
    (\nabla^2+k^2)\hat G_k(\bm r,\bm r')=-4\pi\vd(\bm r-\bm r').
\]
%不同于时域中的Green函数$G(\bm r,t;\bm r',t')$
$\hat G_k$是关于源点$\bm r'$球对称的，定义$\bm R:=\bm r-\bm r'$可得
\[
    \frac1R\dd[2]R(R\hat G_k)+k^2\hat G_k=-4\pi\vd(\bm R),
\]
当$R\neq 0$时， 
\[
    \pp[2]R(R\hat G_k)+k^2(R\hat G_k)=0\implies \hat G_k^\pm(R)=\frac{\e{\pm\i kR}}R
\]
%即Green函数可以写成
...
回到时域下的Green函数，满足
\[
    \biggkh{\lapla-\frac1{c^2}\pp[2]t}G^\pm(\bm r,t;\bm r',t')=-4\pi\vd(\bm r-\bm r')\vd(t-t'),
\]
对$t$进行Fourier变换， 
\[
    (\lapla+k^2)\hat G(\bm r,\omega;\bm r',t')=-4\pi\vd(\bm r-\bm r')\e{\i\omega t'}.
\]
可得 
\[
    \hat G^\pm(\bm r,\omega;\bm r',t')=\hat G_k^\pm(R)\e{\i\omega t'}=\frac{\e{\i(\pm kR+\omega t')}}R,
\]
进行Fourier反变换，
\[
...
即
\begin{equation}
    G^\pm(\bm r,t;\bm r',t')=\frac1{\abs{\bm r-\bm r'}}\vd\biggkh{t'-\Bigkh{t\mp\frac{\abs{\bm r-\bm r'}}c}}.
\end{equation}
分别为推迟(retarded)和超前(advanced) Green函数。$\delta$函数的自变量表明：在$\bm r$点于时间$t$观测的效应是由距离为$\bm R$的源在%推迟(或超前)
时间为$t'=t\mp R/c$时的作用引起的。\footnote{课上的理解是：无穷远处产生的源经过$\bm r$后仍需要$R/c$的时间去经过$\bm r'$。}
从而有源波动方程\eqref{eqn:DAlembertianPsi=4pif}的两个解为 
\[
    \Psi^\pm(\bm r,t)=\int G^\pm(\bm r,t;\bm r',t')f(\bm r',t')\d V'\nd t',
\]
我们考虑一个在时间和空间中作局域分布的源$f(\bm r',t')$，这个源分布只在$t'=0$前后一段有限的时间间隔内不为零。则在无源的波动方程仅仅满足其次波动方程
\[
...
\begin{compactenum}
	...
	\[
        \Psi(\bm r,t)=\Psi_\text{in}(\bm r,t)+\int G^+(\bm r,t;\bm r',t')f(\bm r',t')\d V'\nd t';
    \]
	\item $t\to+\infty$，存在一个特定的“稳态”波$\Psi_\text{out}$，
	\[
        \Psi(\bm r,t)=\Psi_\text{out}(\bm r,t)+\int G^-(\bm r,t;\bm r',t')f(\bm r',t')\d V'\nd t',
    \]
\end{compactenum}
最常见的物理情景是$\Psi_\text{in}=0$%“超前”这个概念总是太科幻了，
\begin{equation}
    \Psi(\bm r,t)=\int\frac{f(\bm r',t')}{\abs{\bm r-\bm r'}}\d V',\quad t'=t-\frac{\abs{\bm r-\bm r'}}c.
\end{equation}
...
\begin{subequations}
    \begin{align}
        \Phi(\bm r,t)&=\frac1{4\pi\varepsilon_0}\int\frac{\rho(\bm r',t')}{\abs{\bm r-\bm r'}}\d V',\\
        \bm A(\bm r,t)&=\frac{\mu_0}{4\pi}\int\frac{\bm J(\bm r',t')}{\abs{\bm r-\bm r'}}\d V'.
    \end{align}
\end{subequations}
% 由
...
\begin{subequations}
    \begin{align}
        \bm E(\bm r,t)&=\frac1{4\pi\varepsilon_0}\int\biggfkh{-[\nabla'\rho]_\ret-\frac1{c^2}\pv{\bm J}{t'}}\frac{\d V'}{\abs{\bm r-\bm r'}},\\
        \bm B(\bm r,t)&=\frac{\mu_0}{4\pi}\int\frac{[\nabla'\times\bm J]_\ret}{\abs{\bm r-\bm r'}}\d V',
    \end{align}
\end{subequations}
% 注意上式中$\nabla'\rho$和$\nabla'\times\bm J$是先求梯度和散度再令$t'=t-R/c$。
Jackson用$[\cdot]_\text{ret}$表示赋值$t'=t-R/c$，这样的好处是可以有效区分$\nabla[f]_\text{ret}\neq[\nabla f]_\text{ret}$，因为前者的$t$与$\bm r'$有关，进一步的：
\begin{align*}
...
\begin{subequations}
    \begin{align}
        \bm E(\bm r,t)&=\frac1{4\pi\varepsilon_0}\int\biggfkh{\biggkh{\frac{\rho(\bm r',t')}R+\frac1c\pv\rho{t'}}\uvec R-\frac1{c^2}\pv{\bm J}{t'}}\frac{\d V'}R,\\
        \bm B(\bm r,t)&=\frac{\mu_0}{4\pi}\int\biggfkh{\frac{\bm J(\bm r',t')}R+\frac1c\pv{\bm J}{t'}}\times\frac{\nvec R}R\d V'.
    \end{align}
\end{subequations}
上式称为Jefimenko对Coulomb定律和Biot-Savart定律的推广。
若$\rho$和$\bm J$是与时间无关的，则可重新得到静电场和静磁场的表达式\eqref{eqn:E(r)}和\eqref{eqn:B(x)}。
\begin{example}{电荷量变化的点电荷}{time-varying point charge}
    空间中原点处一点电荷，其电荷量随时间变化
    \[
        \rho(\bm r',t')=Q(t')\vd(\bm r'),
    \]
    则
    \[
...
\begin{example}{阶跃电流}{current}
    ...
    电流密度为
    \[
        \bm J(\bm r',t')=I(t')\vd(x')\vd(y')\uvec z.
    \]
    矢量势
    \[
        \bm A(\bm r,t)=\frac{\mu_0}{4\pi}\uvec z\int\iti I\Bigkh{t-\frac Rc}\frac{\d z'}R,\quad R=\sqrt{\rho^2+(z-z')^2},
    \]
...
宏观场被定义为微观场的空间平均：
\[
    \bm E(\bm r,t)=\ave{\bm e(\bm r,t)},\quad \bm B(\bm r,t)=\ave{\bm b(\bm r,t)},
\]
其中空间平均值是相对于测试函数$f(\bm r)$而言的，其定义为：
\[
    \ave{F(\bm r,t)}:=\int F(\bm r-\bm r',t)f(\bm r')\d V',
\]
...
\begin{align*}
    \varrho_\text{free}(\bm r,t)&=\sum_jq_j\vd(\bm r-\bm r_j);\\
    \varrho_\text{bound}(\bm r,t)&=\sum_n\varrho_n(\bm r,t).
\end{align*}
$\varrho_n$是第$n$个分子的电荷密度，分子质心坐标为$\bm r_n$，分子电荷相对质心坐标为$\bm r_{nj}$，则其平均值为
\[
    \ave{\varrho_n(\bm r,t)}=\sum_{j(n)}q_jf(\bm r-\bm r_n-\bm r_{nj}),
\]
因为$\bm r_{nj}$具有原子线度的数量级，适合在$(\bm r-\bm r_n)$处Taylor展开： 
\[
    f(\bm r-\bm r_n-\bm r_{nj})=f(\bm r-\bm r_n)-\bm r_{nj}\cdot\nabla f(\bm r-\bm r_n)+\frac12\sum_{\alpha,\beta}(\bm r_{nj})_\alpha(\bm r_{nj})_\beta\pw f{x_\alpha}{x_\beta}(\bm r-\bm r_n)+\cdots,
\]
式中各项分别对应分子电荷、偶极矩、四极矩……
\begin{align*}
    \ave{\varrho_n(\bm r,t)}={}&q_nf(\bm r-\bm r_n)-\bm p_n\cdot\nabla f(\bm r-\bm r_n)+\frac16\sum_{\alpha,\beta}(Q_n)_{\alpha\beta}\pw f{x_\alpha}{x_\beta}(\bm r-\bm r_n)+\cdots
\end{align*}
将上式各项表示成空间平均的形式：
\begin{align*}
    \ave{\varrho_n(\bm r,t)}={}&\ave{q_n\vd(\bm r-\bm r_n)}-\div\ave{\bm p_n\vd(\bm r-\bm r_n)}+\frac16\sum_{\alpha,\beta}\pw{}{x_\alpha}{x_\beta}\ave{(Q_n)_{\alpha\beta}\vd(\bm r-\bm r_n)}+\cdots
\end{align*}
就平均而言，可以把分子看作位于分子质心(也可选取其他固定点)上的点多极子的集合。对所有分子求和，得到
\[
    \ave{\varrho(\bm r,t)}=\rho(\bm r,t)-\div\bm P(\bm r,t)+\sum_{\alpha,\beta}\pw{}{x_\alpha}{x_\beta}Q_{\alpha\beta}(\bm r,t)+\cdots
\]
其中$\rho,\bm P,Q_{\alpha\beta}$分别是宏观电荷密度、宏观电极化强度、宏观电四极矩密度……
...
宏观电位移矢量定义为
\[
    D_\alpha:=\varepsilon_0E_\alpha+P_\alpha-\sum_\beta\pv{Q_{\alpha\beta}}{x_\beta}+\cdots
\]
前两项是熟知的结果，更高阶项原则上是存在的，但几乎可以忽略不计。
\paragraph{电流密度}
$\ave{\bm j}$的推导比$\ave{\varrho}$的处理复杂的多，在这里仅给出结果。定义分子磁矩和宏观磁化强度分别为
\[
    \bm m_n=\frac12\sum_{j(n)}q_j\bm r_{jn}\times\bm v_{jn},\quad\bm M(\bm r,t)=\sum_n\bm m_n\vd(\bm r-\bm r_n),
\]
宏观电流密度
\[
    \bm J(\bm r,t)=\ave{\bm j_\text{free}+\bm j_\text{bound}},\quad\bm j_\text{free}(\bm r,t):=\sum_jq_j\bm v_j\vd(\bm r-\bm r_j).
\]
...
则对于电荷和电流的连续分布，在有限体积$V$中，各场做功的总功率为
\[
...
\begin{compactenum}
    ...
    \[
        u=w_\elc+w_\mag=\frac12(\bm E\cdot\bm D+\bm H\cdot\bm B);%\frac12\int_V\bm E\cdot\bm D\d V,\quad w_B=\frac12\int_V\bm H\cdot\bm B\d V.
    \]
	\item 宏观介质的电磁性质是线性的，
    \[
        \bm D=\varepsilon\bm E,\quad\bm H=\bm B/\mu;
...
\begin{align*}
    \dv{E_\text m}t+\dv{E_\text f}t&=\int_V\bm J\cdot\bm E\d V+\dd t\int_Vu\d V=-\oint_{\p V}\bm S\cdot\d\bm A.
\end{align*}
若空间中没有电荷（源）
\[
...
\begin{equation}
    T_{\alpha\beta}:=\varepsilon_0\Bigfkh{E_\alpha E_\beta+c^2B_\alpha B_\beta-\frac12(E^2+c^2B^2)\vd_{\alpha\beta}}.
\end{equation}
则
\[
    \dd t({\bm P_\text m}+{\bm P_\text f})_\alpha=\oint_{\p V}\sum_\beta T_{\alpha\beta}\hat n_\beta\d A.
\]
\paragraph{角动量守恒}
角动量与动量有关：
\[
    \bm L=\bm r\times\bm P,
\]
故结论与动量守恒相似，仅仅是所有表达式左乘$\bm r$罢了。而且也涉及并矢这种不是很优雅的表达，特略。
...
\begin{align*}
    \bm E(\bm r,t)&=\int\iti\hatbm E(\bm r,\omega)\e{-\i\omega t}\d\omega,\\
    \bm D(\bm r,t)&=\int\iti\hatbm D(\bm r,\omega)\e{-\i\omega t}\d\omega;
\end{align*}
在此我们仍假设介质是各向同性的，从而
\[
    \hatbm D(\bm r,\omega)=\varepsilon(\omega)\hatbm E(\bm r,\omega),
\]
其中$\varepsilon$是复数，而场$\bm E(\bm r,t)$必然仍是实数，因此%简单的将积分变量$\omega$变为$-\omega$可得
\[
    \hatbm E(\bm r,-\omega)=\hatbm E\cj(\bm r,\omega),\quad\hatbm D(\bm r,-\omega)=\hatbm D\cj(\bm r,\omega),
\]
从而$\varepsilon(-\omega)=\varepsilon\cj(\omega)$，色散的性质表面：$\bm D(\bm r,t)$和$\bm E(\bm r,t)$在时间上的联系是非局域性的。因此$\bm E\cdot(\p\bm D/\p t)\neq\p(\bm E\cdot\bm D/2)/\p t$，我们先不考虑空间相关性，利用Fourier变换
\begin{align*}
...
\[
    \bm E(\bm r,t)=\Re\bigfkh{\bm E(\bm r)\e{-\i\omega t}},
\]
其中$\bm E(\bm r)$一般是复的，有相位的信息。
%因此
%\bm J\cdot\bm E=\frac12\Re\bigfkh{\bm J\cj(\bm r)\cdot\bm E(\bm r)+\bm J(\bm r)\cdot\bm E(\bm r)\e{-\i 2\omega t}},
谐振场的Maxwell方程组可以写成
\begin{subequations}
    \begin{align}
        \div\bm D&=\rho,\\
...
\begin{subequations}
    \begin{align}
        w_\elc&=\frac14\bm E\cdot\bm D\cj,\\
        w_\mag&=\frac14\bm B\cdot\bm H\cj.
    \end{align}
\end{subequations}
故简谐场的Poynting定律为
\begin{equation}
    \frac12\int_V\bm J\cj\cdot\bm E\d V+2\i\omega\int_V(w_\elc-w_\mag)\d V+\oint_{\p V}\bm S\cdot\d\bm A=0.
\end{equation}
实部给出了时间平均量的能量守恒；虚部与反应性(或储存的)能量及其交替流动有关。如果能量密度$w_\elc$和$w_\mag$体积分为实数，就像具有无损耗介质和完美导体的系统一样，实部将是：
\[
    \frac12\int_V\Re(\bm J\cj\cdot\bm E)\d V+\oint_{\p V}\Re(\bm S)\cdot\d\bm A=0.
\]
...
\begin{definition}{空间反演}{spatial reflection}
    空间反演(spatial reflection)变换或称宇称(parity)变换是指
    \[
        P:\bm r_i\mapsto -\bm r_i,
    \]
...
\begin{corollary}
    我们可以对熟知的一些矢量进行归类：
    \begin{compactitem}
    	\item 真矢量：坐标$\bm r$、速度$\bm v$、动量$\bm p$、电场$\bm E$；
    	\item 赝矢量：角动量$\bm L$、力矩$\bm N$、磁场$\bm B$。
    \end{compactitem}
    此外还有真标量(如质量$m$、电荷$q$)和赝标量(如混合积$\bm A\cdot(\bm B\times\bm C)$)之分。
\end{corollary}

\begin{remark}
    Poynting矢量是真矢量与赝矢量叉乘($\bm S=\bm E\times\bm H$)，但其仍是真矢量。
\end{remark}

\begin{definition}{时间反演}{time reversal}
    时间反演(time reversal)即
    \[
        t'=-t,
    \]
