...
\[
    \Phi(\bm r,t)=\frac1{4\pi\varepsilon_0}\int_V\frac{\rho(\bm r',t)}{|\bm r-\bm r'|}\d V'
 \]
...
即
\begin{equation}
    G^\pm(\bm r,t;\bm r',t')=\frac1{|\bm r-\bm r'|}\vd\biggkh{t'-\Bigkh{t\mp\frac{|\bm r-\bm r'|}c}}.
\end{equation}
...
\begin{equation}
    \Psi(\bm r,t)=\int\frac{f(\bm r',t')}{|\bm r-\bm r'|}\d V',\quad t'=t-\frac1c|\bm r-\bm r'|.
\end{equation}
...
\begin{subequations}
    \begin{align}
        \Phi(\bm r,t)&=\frac1{4\pi\varepsilon_0}\int\frac{\rho(\bm r',t')}{|\bm r-\bm r'|}\d V',\\
        \bm A(\bm r,t)&=\frac{\mu_0}{4\pi}\int\frac{\bm J(\bm r',t')}{|\bm r-\bm r'|}\d V'.
    \end{align}
\end{subequations}
由
...
\begin{align*}
    \ave{\varrho_n(\bm r,t)}={}&q_nf(\bm r-\bm r_n)-\bm p_n\cdot\nabla f(\bm r-\bm r_n)+\frac16\sum_{\alpha,\beta}(\mathcal Q_n)_{\alpha\beta}\pw f{x_\alpha}{x_\beta}(\bm r-\bm r_n)+\cdots
\end{align*}
将上式各项表示成空间平均的形式：
\begin{align*}
    \ave{\varrho_n(\bm r,t)}={}&\ave{q_n\vd(\bm r-\bm r_n)}-\div\ave{\bm p_n\vd(\bm r-\bm r_n)}+\frac16\sum_{\alpha,\beta}\pw{}{x_\alpha}{x_\beta}\ave{(\mathcal Q_n)_{\alpha\beta}\vd(\bm r-\bm r_n)}+\cdots
\end{align*}
就平均而言，可以把分子看作位于分子质心(也可选取其他固定点)上的点多极子的集合。对所有分子求和，得到
\[
    \ave{\varrho(\bm r,t)}=\rho(\bm r,t)-\div\bm P(\bm r,t)+\sum_{\alpha,\beta}\pw{}{x_\alpha}{x_\beta}\mathcal Q_{\alpha\beta}(\bm r,t)+\cdots
\]
...
故简谐场的Poynting定律为
\[
    \frac12\int_V\bm J\cj\cdot\bm E\d V+2\i\omega\int_V(w_\elc-w_\text m)\d V+\oint_{\p V}\bm S\cdot\d\bm A=0.
\]
