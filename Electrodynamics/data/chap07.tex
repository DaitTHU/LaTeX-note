...
\begin{equation}
    \dv{P_\text{loss}}a=-\frac12\Re[\uvec n\cdot(\bm E\times\bm H\cj)]=\frac{\mu\c\omega\delta}4\abs{\bm H_\parallel}^2.
\end{equation}
可把这个结果简单地解释为导体内的Ohm损失。
每单位体积内Ohm损失率的时间平均为
% 由Ohm定律，导体表面附件有电流密度$\bm J=\sigma\bm E\c$，而
\[
    \dv{P_\text{loss}}V=\frac12\bm J\cdot\bm E\cj=\frac\sigma 2\abs{\bm J}^2.
\]
...
\begin{equation}
    \label{eqn:dPloss/da}
    \dv{P_\text{loss}}a=\frac1{2\sigma\delta}\abs{\bm\jmath\eff}^2.
\end{equation}
...
\begin{center}
    \includegraphics[page=\pagenumref{tikz:rectangular waveguide}]{figures/tikz/layouts.pdf}
...
在内径为$a$的圆形波导中，
\begin{center}
    \includegraphics[page=\pagenumref{tikz:circle waveguide}]{figures/tikz/layouts.pdf}
...
\begin{center}
    \includegraphics[page=\pagenumref{tikz:dielectric waveguide}]{figures/tikz/layouts.pdf}
...
\section{腔内的功率损失}
对于理想导体，$\abs{E(\omega)}$在各个$\omega_i$上是一个个$\delta$函数；而对于良导体，则会有一定的半高宽。
