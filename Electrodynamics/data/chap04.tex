...
\begin{theorem}{Biot-Savart定律}{Biot and Savart Law}
    原点处电流元$I\d\bm\ell$在$\bm r$处激发的磁感应强度元$\d\bm B$为
    \begin{equation}
        \label{eqn:B-S law}
        \d\bm B=k\frac{I\d\bm\ell\times\bm r}{|\bm r|^3},\quad k=\frac{\mu_0}{4\pi},
    \end{equation}
...
\[
    \bm B=k\frac{q\bm v\times\bm r}{|\bm r|^3}.
\]
...
\begin{example}{闭合回路之间的力}{F of 2 closed lines}
    ...
    \[
        \bm F_{12}=\frac{\mu_0}{4\pi}I_1I_2\oint\oint\frac{\d\bm\ell_1\times(\d\bm\ell_2\times\bm r_{12})}{|\bm r_{12}|^3};
    \]
    利用\eqref{eqn:bac-cab}将双叉乘化为更对称的形式：
    %\bm A\times(\bm B\times\bm C)=\bm B(\bm A\cdot\bm C)-\bm C(\bm A\cdot\bm B)
    \[
        \bm F_{12}=\frac{\mu_0}{4\pi}I_1I_2\biggfkh{\oint\cancel{\oint\frac{\bm r_{12}\cdot\d\bm\ell_1}{|\bm r_{12}|^3}}\d\bm\ell_2-\oint\oint\frac{\bm r_{12}}{|\bm r_{12}|^3}(\d\bm\ell_1\cdot\d\bm\ell_2)}.
    \]
    第一项的环路积分为0：
    \[
        \oint\frac{\bm r_{12}\cdot\d\bm\ell_1}{|\bm r_{12}|^3}=\oint\frac{\d x_{12}}{x_{12}^2}=0.
    \]
...
\begin{align}
    \label{eqn:B(x)}
    \bm B(\bm r)&=\frac{\mu_0}{4\pi}\int_V\bm J(\bm r')\times\frac{\bm r-\bm r'}{|\bm r-\bm r'|^3}\d V'\\
    &=\frac{\mu_0}{4\pi}\curl\int_V\frac{\bm J(\bm r')}{|\bm r-\bm r'|}\d V'.
\end{align}
...
\begin{align*}
    \curl\biggfkh{\curl\frac{\bm J(\bm r')}{|\bm r-\bm r'|}}&=\nabla\biggfkh{\div\frac{\bm J(\bm r')}{|\bm r-\bm r'|}}-\lapla\frac{\bm J(\bm r')}{|\bm r-\bm r'|}\\
    &=\nabla\biggfkh{\frac{\cancel{\div\bm J(\bm r')}}{|\bm r-\bm r'|}+\bm J(\bm r')\cdot\nabla\frac1{|\bm r-\bm r'|}}-\bm J(\bm r')\lapla\frac1{|\bm r-\bm r'|}.
\end{align*}
则
\[
    \curl\bm B=\frac{\mu_0}{4\pi}\biggfkh{-\nabla\int_V\bm J(\bm r')\cdot\nabla'\frac1{|\bm r-\bm r'|}\d V'+4\pi\bm J(\bm r)}
\]
对积分项进行分部积分：
\[
    \int_V\bm J(\bm r')\cdot\nabla'\frac1{|\bm r-\bm r'|}\d V'=\oint_{\p V}\frac{\cancel{\bm J(\bm r')\cdot\uvec n'}}{|\bm r-\bm r'|}\d A'-\int_V\frac{\cancel{\nabla'\cdot\bm J(\bm r')}}{|\bm r-\bm r'|}\d V'=0,
\]
\iffalse
\begin{align*}
    \curl\bm B&=\frac{\mu_0}{4\pi}\curl\biggfkh{\curl\int_V\frac{\bm J(\bm r')}{|\bm r-\bm r'|}\d V'}\\
    &\qquad\textcolor{gray}{\curl(\curl\bm A)=\nabla(\div\bm A)-\lapla\bm A}\\
    &=\frac{\mu_0}{4\pi}\biggfkh{\nabla\int_V\div\frac{\bm J(\bm r')}{|\bm r-\bm r'|}\d V'-\int_V\lapla\frac{\bm J(\bm r')}{|\bm r-\bm r'|}\d V'}\\
    &\qquad\textcolor{gray}{\div(\varphi\bm A)=A\cdot\nabla\varphi+\varphi\div\bm A}\\
    &=\frac{\mu_0}{4\pi}\biggfkh{\nabla\int_V\biggkh{\bm J(\bm r')\cdot\nabla\frac1{|\bm r-\bm r'|}+0}\d V'-\int_V\bm J(\bm r')\lapla\frac1{|\bm r-\bm r'|}\d V'}\\
    %=\frac{\mu_0}{4\pi}\biggfkh{-\nabla\int_V\bm J(\bm r')\cdot\nabla'\frac1{|\bm r-\bm r'|}\d V'+4\pi\bm J(\bm r)}\\
    &\qquad\textcolor{gray}{\int_V\bm A\cdot\nabla f\d V=-\int_Vf\div\bm A\d V+\oint_{\p V}f\bm A\cdot\d\bm A}\\
    &=\mu_0\bm J(\bm r)+\frac{\mu_0}{4\pi}\nabla\biggfkh{\int_V\frac{\cancel{\nabla'\cdot\bm J(\bm r')}}{|\bm r-\bm r'|}\d V'-\cancel{\oint_{\p V}\frac{\bm J(\bm r')\cdot\uvec n'}{|\bm r-\bm r'|}\cdot\d A'}};
\end{align*}
\fi
由此得到第二个微分方程：
\begin{equation}
...
两边取$S$的面积分
\[
    \int_S\curl(\bm B\cdot\uvec n)\d A=\mu_0\int_S\bm J\cdot\d\bm A.
\]
...
\begin{equation}
    \label{eqn:A-J}
    \bm A(\bm r):=\frac{\mu_0}{4\pi}\int\frac{\bm J(\bm r')}{|\bm r-\bm r'|}\d V',
\end{equation}
在Coulomb规范下，
...
\begin{example}{环形电流回路的磁场}{B of loop current}
    ...
    \begin{align*}
        A_\phi&=\frac{\mu_0}{4\pi}\int\frac{J_\phi}{|\bm r-\bm r'|}\d V'\\
    ...
    \end{alignat*}
    \begin{center}
        \includegraphics[width=7cm]{figures/ellipseKE.pdf}
...
\begin{equation}
    \label{eqn:A approx}
    A_\alpha(\bm r)=\frac{\mu_0}{4\pi}\biggfkh{\frac1{|\bm r|}\cancel{\int_V J_\alpha(\bm r')\d V'}+\frac{\bm r}{|\bm r|^3}\cdot\int_V J_\alpha(\bm r')\bm r'\d V+\cdots}
    \tag{$\ast$}
\end{equation}
下面我们用一些数学上的小tricks：
\[
...
\begin{equation}
    \bm A(\bm r)=\frac{\mu_0}{4\pi}\frac{\bm m\times\bm r}{|\bm r|^3}.
\end{equation}
进而磁感应强度与电偶极矩电场\eqref{eqn:edipole-E with delta}形式相似：
\begin{equation}
    \bm B(\bm r)=\frac{\mu_0}{4\pi}\biggfkh{\frac{3\uvec n(\bm m\cdot\uvec n)-\bm m}{|\bm r|^3}+\frac{8\pi}3\bm m\vd(\bm r)}.
\end{equation}
...
力矩与作用在磁偶极子上的力矩表达式相同：
\[
    \bm N=\int_V\bm r'\times(\bm J\times\bm B)\d V'\simeq\bm m\times\bm B(\bm 0),
\]
上式也可作为定义磁感应强度的一种方法。
...
\end{equation}
在$\bm r'$由一个小体积的$\D V$产生的矢量势
\[
    \D\bm A(\bm r)\simeq\frac{\mu_0}{4\pi}\biggfkh{\frac{\bm J(\bm r)'}{|\bm r-\bm r'|}+\frac{\bm M(\bm r')\times(\bm r-\bm r')}{|\bm r-\bm r'|^3}}\D V.
\]
其中宏观磁矩密度
\[
    \bm M(\bm r):=\sum_iN_i\ave{\bm m_i},
\]
积分之
\[
    \bm A(\bm r)=\frac{\mu_0}{4\pi}\int_V\frac{\bm J(\bm r')+\nabla'\times\bm M(\bm r')}{|\bm r-\bm r'|}\d V'
\]
因此 
\[
...
\end{itemize}
\begin{center}
    \includegraphics[width=0.5\linewidth]{figures/hysteresis.pdf}
...
若无边界条件，则有解
\[
    \Phi_\text M=-\frac1{4\pi}\int_V\frac{\nabla'\cdot\bm M(\bm r')}{|\bm r-\bm r'|}\d V',
\]
若$\bm M$是局域的，则在$x\gg x'$的地方
\[
    \Phi_\text M\simeq\frac{\bm m\cdot\bm r}{4\pi r^3},\quad\bm m=\int_V\bm M\d V.
\]
但是我们仍需面对有边界条件的情形，在边界$\p V$处$\bm M$瞬间变为0，运用静电学中的结论：
\begin{equation}
    \label{eqn:PhiM with boundary}
    \Phi_\text M=-\frac1{4\pi}\int_V\frac{\nabla'\cdot\bm M(\bm r')}{|\bm r-\bm r'|}\d V'+\frac1{4\pi}\oint_{\p V}\frac{\bm M(\bm r')\cdot\uvec n'}{|\bm r-\bm r'|}\d A',
\end{equation}
...
\begin{equation}
    ...
    \bm A=\frac1{4\pi}\int_V\frac{\nabla'\times\bm M(\bm r')}{|\bm r-\bm r'|}\d V'+\frac1{4\pi}\oint_{\p V}\frac{\bm M(\bm r')\times\uvec n'}{|\bm r-\bm r'|}\d A',
\end{equation}
...
\begin{example}{均匀磁化球}{uniform magnetized sphere}
    ...
    \[
        \Phi_\text M(r,\theta)=0+\frac1{4\pi}\oint_{r'=a}\frac{M_0\cos\theta'}{|\bm r-\bm r'|}\cdot a^2\d\Omega',
    \]
    进行球谐函数展开，式\eqref{eqn:1/|r-r'|=YY}仅有$\ell=1,\enspace m=0$项保留，且
    \[
    ...
    \[
        \Phi_\text M=\frac{M_0a^3}3\frac{\cos\theta}{r^2}=\frac{\bm m\cdot\bm r}{4\pi r^3},\quad\bm m=\frac{4\pi}3a^3\bm M.
    \]
