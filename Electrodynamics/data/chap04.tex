...
\begin{corollary}
    对于离散情形，
    \begin{equation}
        \bm J(\bm r)=\sum_i q_i\bm v_i\vd(\bm r-\bm r_i).
    \end{equation}
\end{corollary}
...
\begin{corollary}
    Lorentz力的功率$\bm F\cdot\bm v=q(\bm v\times\bm B)\cdot\bm v=q(\bm v\times\bm v)\cdot\bm B=0$，即磁场不对电荷做功。
\end{corollary}
...
\begin{theorem}{Ampère定律}{Ampere's law}
    磁感应强度沿闭合曲线$\p S$的环量与穿过$S$的电流$I$的关系为：
    \begin{equation}
        \label{eqn:Ampere}
...
\begin{proof}
    由磁场的旋度\eqref{eqn:curlB}
    \[
        \int_{\p S}\bm B\cdot\d\bm\ell=\int_S(\curl\bm B)\cdot\d\bm A=\mu_0\int_S\bm J\cdot\d\bm A=\mu_0I.
        \qedhere
    \]
\end{proof}
