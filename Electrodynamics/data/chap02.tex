...
\begin{example}{接地无限平面导体的镜像电荷}{method of images, infinite plane conductor}
    ...
    \begin{center}
        ...
        \label{fig:method of images, plane}
    \end{center}
    因此电势分布为
    \begin{equation}
        \Phi(\bm r)=\frac q{4\pi\varepsilon_0}\biggkh{\frac1{\abs{\bm r-\bm d}}-\frac1{\abs{\bm r+\bm d}}}.
    \end{equation}
\end{example}
...
\begin{example}{接地导体球的镜像电荷}{method of images, grounded sphere conductor}
    ...
    \begin{center}
        ...
        \label{fig:method of images, sphere conductor grounded}
    \end{center}
    ...
    \begin{equation}
        \Phi(\bm r)=\frac q{4\pi\varepsilon_0}\biggkh{\frac1{\abs{\bm r-\bm d}}-\frac ad\frac1{\abs{\bm r-\bm d'}}},\quad\bm d'=\Bigkh{\frac ad}^2\bm d.
    \end{equation}
    \tcblower
    接地导体球外表面的面电荷密度为($\p V$上的法向量$\uvec n'$指向球内，即$-r$方向)
    \begin{equation}
...
\begin{example}{电像法处理匀强电场中的导体球问题}{method of images, sphere conductor in uniform E}
    ...
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:uniform E approx pm q}]{figures/tikz/layouts.pdf}
        \captionof{figure}{正负电荷近似匀强电场}
        \label{fig:uniform E approx pm q}
    \end{center}
...
对于球形Dirichlet边界条件，为了使球面上的$G(\bm r,\bm r')=0$，可借助镜像电荷的思想，取
\begin{equation}
    \label{eqn:sphere Green function}
    G(\bm r,\bm r')=\frac1{\abs{\bm r-\bm r'}}-\frac a{r'}\frac1{\abs{\bm r-\bm r''}},\quad\bm r''=\Bigkh{\frac{a}{r'}}^2\bm r'.
\end{equation}
令$\gamma$表示$\bm r,\bm r'$夹角，则
\[
    G(\bm r,\bm r')=\frac1{(r^2+r'^2-2rr'\cos\gamma)^{1/2}}-\frac a{(r^2r'^2+a^4-2a^2rr'\cos\gamma)^{1/2}},
\]
式\eqref{eqn:Dirichlet Green}中还需要$\p G/\p n'$，若感兴趣的区域为球外，则$n'$指向球内，
\begin{equation}
    \label{eqn:sphere dGreen function}
    \edg{\pv G{n'}}_{r'=a}=-\edg{\pv G{r'}}_{r'=a}=-\frac{r'^2-a^2}{a(r'^2+a^2-2ar'\cos\gamma)^{3/2}}.
\end{equation}
...
\begin{example}{两半球具有不同电势的导体球}{2 hemisphere conductor at different potentials}
    ...
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:2 hemisphere}]{figures/tikz/layouts.pdf}
        \captionof{figure}{两半球具有不同电势的导体球}
        \label{fig:2 hemisphere}
    \end{center}
    使用式\eqref{eqn:sphere Laplace solution}，
    \begin{align*}
        \Phi(\bm r)&=\frac V{4\pi}\int_0^{2\pi}\biggkh{\int_0^1-\int_{-1}^0}\frac{a(r^2-a^2)}{(r^2+a^2-2ar\cos\gamma)^{3/2}}\d(\cos\theta')\nd\phi'%\\
    ...
    \begin{align*}
        \Phi(\bm r)&=\frac{Va(r^2-a^2)}{4\pi(r^2+a^2)^{3/2}}\int_0^{2\pi}\int_0^1\Bigl[(1-2\alpha\cos\gamma)^{-3/2}-(1+2\alpha\cos\gamma)^{-3/2}\Bigr]\d(\cos\theta')\nd\phi'
    \end{align*}
    ...
    按项积分得到
    \[
        \Phi(\bm r)=\frac{3Va^2}{2r^2}\frac{r^3(r^2-a^2)}{(r^2+a^2)^{5/2}}\cos\theta\biggfkh{1+\frac{35}{24}\frac{a^2r^2}{(r^2+a^2)^2}(3-\cos^2\theta)+\cdots}.
    \]
    若$r\gg a$，则
    \begin{equation}
...
\begin{definition}{正交函数}{orthogonal function}
    ...
    \begin{equation}
        \int_a^bU_m\cj(\xi)U_n(\xi)\d\xi=\delta_{mn}.
    \end{equation}
    则称为正交函数。其中$\delta_{mn}$为Kronecker符号。
\end{definition}

\begin{theorem}
    {完备正交函数展开}{complete orthogonal function expansion}
    若有一组完备的正交函数集$\{U_i(\xi)\}$，则在区间$(a,b)$上任意平方可积的函数$f(\xi)$均可被展开成正交函数的线性组合
    \[
        f(\xi)=\sum_{i=1}^\infty a_iU_i(\xi),\quad a_i:=\int_a^bU_i\cj(\xi)f(\xi)\d\xi.
    \]
    完备关系(completeness relation)为
    \begin{equation}
        \sum_{i=1}^\infty U_i\cj(\xi')U_i(\xi)=\delta(\xi'-\xi).
    \end{equation}
\end{theorem}
...
\begin{proof}
    直接代入系数$a_i$
    \[
        f(\xi)=\int_a^b\biggfkh{\sum_{i=1}^\infty U_i\cj(\xi')U_i(\xi)}f(\xi')\d\xi'.
        \qedhere
    \]
\end{proof}
...
\begin{example}{Fourier变换}{Fourier transformation}
    Fourier级数有复变函数的形式
    \[
        f(x)=\frac1{\sqrt\ell}\sum_{n=-\infty}^{+\infty}c_n\e{\i2\pi nx/\ell},
        \quad
        c_n=\frac1{\sqrt\ell}\int_{-\ell/2}^{\ell/2}f(x')\e{-\i2\pi nx'/\ell}\d x'.
    \]
    当$\ell\to\infty$时，
    \[
...
\begin{example}{长方体边界条件下的Laplace方程的解}{solution in cuboid Laplace}
    ...
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:3D rectangular box}]{figures/tikz/layouts.pdf}
    ...
    \]
    其中$\gamma^2=\alpha^2+\beta^2$。
    因为$X(0)=X(a)=0,\,Y(0)=Y(b)=0$，故解为驻波条件：
    \begin{alignat*}{5}
...
\begin{example}{二维直角坐标边界条件}{2D boundary condition in Cartesian coordinates}
    考虑$0<x<a,\;y>0$的边界，显然$\Phi$与$z$无关：
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:2D rectangular hole}]{figures/tikz/layouts.pdf}
        \captionof{figure}{二维矩形井边界条件}
        \label{fig:2D rectangular hole}
    \end{center}
    二维直角坐标Laplace方程为
    \[
        \lapla\Phi=\pv[2]\Phi x+\pv[2]\Phi y+\cancel{\pv[2]\Phi z}=0,
    \]
...
\begin{equation}
    \lapla\Phi=\frac1\rho\pp\rho\biggkh{\rho\pv\Phi\rho}+\frac1{\rho^2}\pv[2]\Phi\phi+\cancel{\pv[2]\Phi z}%=\pv[2]\Phi\rho+\frac1\rho\pv\Phi\rho+\frac1{\rho^2}\pv[2]\Phi\phi
    =0.
\end{equation}
...
\begin{example}{二维极坐标边界条件}{2D boundary condition in polar coordinates}
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:2D angular boundary}]{figures/tikz/layouts.pdf}
        \captionof{figure}{二维极坐标边界条件}
        \label{fig:2D angular boundary}
    \end{center}
...
有限元分析法（finite element analysis, FEA）是一种数值方法，用于求解复杂的工程和物理问题，特别是涉及偏微分方程的连续体力学问题，如结构力学、热传导、电磁场等。
其基本思想是将一个复杂的连续体问题分解成许多简单的子问题，这些子问题在数学上更容易处理。每个子问题对应于一个“有限元”，它是一个小的、简单形状的区域（如三角形或四边形）。然后，在这些简单区域上，通过选择合适的数学近似（如多项式函数），将偏微分方程转化为代数方程。这些代数方程组成了所谓的有限元方程。
...
\begin{equation}
    ...
    \lapla\Phi=\frac1{r^2}\pp r\biggkh{r^2\pv\Phi r}+\frac1{r^2\sin\theta}\pp\theta\biggkh{\sin\theta\pv\Phi\theta}+\frac1{r^2\sin^2\theta}\pv[2]\Phi\phi=0.
\end{equation}
...
\begin{theorem}
    ...
    \begin{subequations}
        \begin{align}
            ...
            \sum_{\ell=0}^\infty P_\ell\cj(x')P_\ell^m(x)&=\delta(x-x').
        \end{align}
    \end{subequations}
...
\begin{corollary}
    对于方位角$\phi$对称的Laplace方程\eqref{eqn:Laplace in spherical}，$m=0$，其解为
    \begin{equation}
...
\begin{example}{点电荷电势}{point charge potential}
    $\bm r'$处的单位点电荷在$\bm r$的产生的电势为
    \begin{equation}
        \label{eqn:1/|r-r'|}
        \frac1{\abs{\bm r-\bm r'}}=\frac1{r_>}\sum_{\ell=0}^\infty\biggkh{\frac{r_<}{r_>}}^\ell P_\ell(\cos\gamma).
    \end{equation}
    其中$r_<:=\min(r,r'),\enspace r_>:=\max(r,r')$，$\gamma$是$\bm r,\bm r'$夹角。
\end{example}
...
\begin{example}{锥形孔内尖端附近的电场特性}{field near the tip of the conical hole}
    ...
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:2D conical boundary}]{figures/tikz/layouts.pdf}
        \captionof{figure}{锥形孔}
        \label{fig:conical hole}
    \end{center}
...
\[
    \dd x\biggfkh{(1-x^2)\dv Px}+\biggfkh{\ell(\ell+1)-\frac{m^2}{1-x^2}}P=0.
\]
为了使解在$[-1,1]$收敛，推导过程见\secref{sec:Legendre polynomial}，要求：
\begin{subequations}
    \begin{align}
        \ell&=0,1,2,\ldots,\\
...
\begin{theorem}
    {连带Legendre函数的正交性}{}
    连带Legendre函数只对固定的$m$或固定的$\ell$正交：
    \begin{subequations}
        \begin{align}
        ...
        \end{align}
    \end{subequations}
    但不能同时对$\ell$和$m$正交，比如$P_1^1$和$P_2^2$并不正交。
\end{theorem}
将角向部分的解$P(\theta)$和$Q(\phi)$合并，
由此得到球谐函数(spherical harmonics)：
\begin{equation}
...
\begin{theorem}
    ...
    球谐函数是正交且完备的：
    \[
        \oint Y_{\ell m}\cj Y_{\ell'm'}\d\Omega=\int_0^{2\pi}\int_0^\pi Y_{\ell m}\cj(\theta,\phi) Y_{\ell'm'}(\theta,\phi)\sin\theta\d\theta\nd\phi=\delta_{\ell\ell'}\vd_{mm'}.
    \]
    \begin{equation}
...
球坐标系下Laplace方程\eqref{eqn:Laplace in spherical}的通解
\begin{equation}
    \Phi(r,\theta,\phi)=\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell\bigfkh{A_{\ell m}r^\ell+B_{\ell m}r^{-(\ell+1)}}Y_{\ell m}(\theta,\phi).
\end{equation}
由$P_\ell(\cos\gamma)$的球谐函数展开式及\eqref{eqn:1/|r-r'|}，点电荷之间电势的球谐函数展开
\begin{equation}
    \label{eqn:1/|r-r'|=YY}
    \frac1{\abs{\bm r-\bm r'}}=\frac{4\pi}{r_>}\sum_{\ell=0}^\infty\biggfkh{\frac1{2\ell+1}\biggkh{\frac{r_<}{r_>}}^\ell\cdot\sum_{m=-\ell}^\ell Y_{\ell m}\cj(\theta',\phi')Y_{\ell m}(\theta,\phi)}.
\end{equation}
...
\begin{example}{圆柱体内电势分布}{potential inside cylinder}
    如下图，圆柱体除顶面外其余面电势均为0.
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:cylindrical boundary}]{figures/tikz/layouts.pdf}
        \captionof{figure}{圆柱形边界条件}
        \label{fig:cylindrical boundary}
    \end{center}
    由边界条件可知，
    \iffalse ：
    \begin{align*}
...
\]
因此有必要确定满足边界条件的Green函数$G(\bm r,\bm r')$。
...
我们希望得到适用于在$r = a$处具有球面边界的外部问题的Green函数的类似展开。对式\eqref{eqn:sphere Green function}中的两项展开\eqref{eqn:1/|r-r'|=YY}，得到
\begin{align}
    \notag
    G(\bm r,\bm r')&=\frac1{\abs{\bm r-\bm r'}}-\frac a{r'}\frac1{\abs{\bm r-(a/r')^2\bm r'}}\\
    \label{eqn:Green YY}
    &=4\pi\sum_{\ell=0}^\infty\biggfkh{\frac1{2\ell+1}\frac1{r_>^{\ell+1}}\biggkh{r_<^\ell-\frac{a^{2\ell+1}}{r_<^{\ell+1}}}\sum_{m=-\ell}^\ell Y_{\ell m}\cj(\theta',\phi')Y_{\ell m}(\theta,\phi)}.
\end{align}
...
\[
    \lapla G(\bm r,\bm r')=-4\pi\vd(\bm r-\bm r').
\]
且$G(\bm r,\bm r')|_{r=a}=0$。
对于球坐标，$\delta$函数可以写成
\begin{align*}
    \delta(\bm r-\bm r')&=\frac1{r^2}\vd(r-r')\vd(\phi-\phi')\vd(\cos\theta-\cos\theta')\\
    &=\frac1{r^2}\vd(r-r')\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell Y_{\ell m}\cj(\theta',\phi')Y_{\ell m}(\theta,\phi).
\end{align*}
将Green函数看做$\bm r$的函数，可以被展开为
\[
    G(\bm r,\bm r')=\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell A_{\ell m}(r|r',\theta',\phi')Y_{\ell m}(\theta,\phi).
\]
由上面三式，可得 
\[
    A_{\ell m}(r|r',\theta',\phi')=g_\ell(r,r')Y_{\ell m}\cj(\theta',\phi')
\]
$g_\ell$满足
\[
...
\begin{example}{同心球边界条件}{concentric spheres boundary}
    ...
    其Green函数的边界条件为
    \[
        G(\bm r,\bm r')|_{r=a}=G(\bm r,\bm r')|_{r=b}=0,
    \]
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:concentric spheres boundary}]{figures/tikz/layouts.pdf}
        \captionof{figure}{同心球边界条件}
        \label{fig:concentric spheres boundary}
    \end{center}
    由$g_\ell(a,r')=g_\ell(b,r')=0$可得
    \[
        g_\ell(r,r')=
        \begin{cases}
    ...
    故Green函数的展开
    \begin{equation}
        G(\bm r,\bm r')=4\pi\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell \frac{Y_{\ell m}\cj(\theta',\phi')Y_{\ell m}(\theta,\phi)}{(2\ell+1)\bigfkh{1-(a/b)^{2\ell+1}}}\biggkh{r_<^\ell-\frac{a^{2\ell+1}}{r_<^{\ell+1}}}\biggkh{\frac1{r_>^{\ell+1}}-\frac{r_>^\ell}{b^{2\ell+1}}}.
    \end{equation}
    \tcblower
    $a\to0,\,b\to\infty$时，变为式\eqref{eqn:1/|r-r'|=YY}
...
\begin{example}{接地球内带电圆环}{charged ring inside grounded sphere}
    ...
    环的电荷密度可以写成
    \[
        \rho(\bm r')=\frac Q{2\pi a^2}\vd(r'-a)\vd(\cos\theta').
    \]
    由方位角的对称性，$m=0$，故
    \begin{align*}
        \Phi(\bm r)&=\frac1{4\pi\varepsilon_0}\int\rho(\bm r')G(\bm r,\bm r')\d V'\\
...
柱坐标系下，
\[
    \lapla G(\bm r,\bm r')=-\frac{4\pi}\rho\vd(\rho-\rho')\vd(\phi-\phi')\vd(z-z').
\]
Green函数可以写成
\[
    G(\bm r,\bm r')=\frac1{4\pi^2}\sum_{m=-\infty}^\infty\int\iti\e{\i m(\phi-\phi')}\e{\i k(z-z')}g_m(k,\rho,\rho')\d k.
\]
...
\]
故$A=4\pi$，我们得到了$1/\abs{\bm r-\bm r'}$的展开 
\begin{equation}
    \frac1{\abs{\bm r-\bm r'}}=\frac1\pi\sum_{m=-\infty}^{+\infty}\int\iti\e{\i m(\phi-\phi')}\e{\i k(z-z')}I_m(k\rho_<)K_m(k\rho_>)\d k.
\end{equation}
...
\begin{equation}
    \lapla\psi(\bm r)+\bigfkh{f(\bm r)+\lambda}\psi(\bm r)=0.
\end{equation}
为了使本征函数$\psi_n(\bm r)$满足边界条件，$\lambda$必须取一系列本征值$\lambda_n$。
%\lapla\psi_n(x)+\bigfkh{f(x)+\lambda_n}\psi_n(x)=0.

本征函数是正交的：
\[
    \int_V\psi_m\cj(\bm r)\psi_n(\bm r)\d V=\vd_{mn}.
\]
本征值$\lambda$的谱可以是离散集，也可以是连续集，或者两者兼而有之。我们假定所有本征函数构成一个完备集。

现在我们希望找到Green函数的方程:
\begin{equation}
    \lapla G(\bm r,\bm r')+\bigfkh{f(\bm r)+\lambda}G(\bm r,\bm r')=-4\pi\vd(\bm r,\bm r'),
\end{equation}
若Green函数与的本征函数$\psi_n(\bm r)$具有相同的边界条件，则可将Green函数展开为一系列本征函数的形式:
\[
    G(\bm r,\bm r')=\sum_n a_n(\bm r')\psi_n(\bm r),
\]
代入得
\[
    \sum_n a_n(\bm r')(\lambda-\lambda_n)\psi_n=-4\pi\vd(\bm r,\bm r'),
\]
因此系数为
\[
    a_n(\bm r')=\frac{4\pi}{\lambda_n-\lambda}\int_V\psi_n\cj(\bm r)\vd(\bm r,\bm r')\d V=4\pi\frac{\psi_n\cj(\bm r')}{\lambda_n-\lambda}.
\]
因此Green函数的本征函数展开式为
\begin{equation}
    G(\bm r,\bm r')=4\pi\sum_n \frac{\psi_n\cj(\bm r')\psi_n(\bm r)}{\lambda_n-\lambda}
\end{equation}
对于连续谱，求和用积分代替。
\begin{example}{$1/\abs{\bm r-\bm r'}$的三维Fourier积分表示}{1/|r-r'| Fourier}
    自由边界下，$f(\bm r)\equiv 0$的特征方程
    \[
        (\lapla+k^2)\psi_{\bm k}(\bm r)=0\implies\psi_{\bm k}(\bm r)=\frac{\e{\i\bm k\cdot\bm r}}{(2\pi)^{3/2}}.
    \]
    则$\lambda=0$时的Green函数为
    \[
        G(\bm r,\bm r')=4\pi\int\frac{\psi_{\bm k}\cj(\bm r')\psi_{\bm k}(\bm r)}{k^2}\d V_k=\frac1{2\pi^2}\int\frac{\e{\i\bm k\cdot(\bm r'-\bm r)}}{k^2}\d V_k.
    \]
\end{example}
