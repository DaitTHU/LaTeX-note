...
电像法(method of images)用于求解一个或多个点电荷在特定边界$\p V$内（或外）的问题。在边界$\p V$形状较好的条件下，可以在$V$外\footnote{镜像电荷必须在$V$外部，否则它们的存在将改变$V$内实际电荷的排列，违反Poisson方程。}（或内）添加适当的点电荷（称为镜像电荷，image charge）来模拟所需的边界条件。
\begin{example}{接地无限大平面导体的镜像电荷}{method of images, infinite plane conductor}
...
\begin{example}{接地导体球的镜像电荷}{method of images, grounded sphere conductor}
    ...
    作用在电荷$q$上的力可以将$\sigma^2/2\varepsilon_0$在整个$\p V$上积分得到。最简单的方法是计算电荷$q$和像电荷$q'$之间的Coulomb力：
    \begin{equation}
...
\exmref{exm:method of images, grounded sphere conductor} 的讨论都是基于点电荷在导体球外。实际上，这个结果同样适用于球内的电荷($y<a$)。唯一需要改变的是\eqref{eqn:grounded sphere conductor-point, sigma}，其中导体外的法向导数现在是径向向内的，这意味着符号的变化。导体球内表面感应电荷为$-q$，与$y$无关。
...
\begin{example}{带电孤立导体球的镜像电荷}{method of images, charged sphere conductor}
    ...
    \begin{equation}
        Q\simeq\frac{qa^2}{4(y-a)^2}\implies y\simeq a\biggkh{1+\frac12\sqrt{\frac qQ}}.
    \end{equation}
\end{example}
...
\paragraph{直角坐标}直角坐标系下，Laplace方程为
\[
    \lapla\Phi=\pv[2]\Phi x+\pv[2]\Phi y+\pv[2]\Phi z=0.
\]
分离变量：假定$\Phi$具有如下的形式解
\[
...
\begin{example}{长方体边界条件下的Laplace方程的解}{solution in cuboid Laplace}
    给定如图边界条件，只有$z=c$的边界有电势$V(x,y)$外，其他面电势均为0，
    \begin{center}
        \begin{tikzpicture}
    ...
    \begin{alignat*}{2}
        \alpha_m&=\frac{m\pi}a,&m&=1,2,3,\ldots\\
        \beta_n&=\frac{n\pi}b,&n&=1,2,3,\ldots\\
...
\begin{example}{二维拐角内和沿棱边的电场与电荷密度}{field and charge density in 2D Corners and along edges}
    \begin{center}
        \begin{tikzpicture}
            ...
            \pic["$\beta$", draw, angle eccentricity=1.4, angle radius=20]{angle=a--o--b};
...
\begin{itemize}
    \item \secref{sec:laplace equation in cylindrical coordinate}和\secref{sec:laplace equation in cylindrical coordinate}讲在球坐标和柱坐标中用分离变量法求解Laplace方程，并介绍了Legendre多项式、球谐函数、Bessel函数等特殊函数的性质；
...
\]
$P_\ell^{-m}$和$P_\ell^m$是线性相关的，因此后式约定$m>0$：
\[
...
\begin{center}
    ...
    \captionof{figure}{前几个第一类Bessel函数}
    \includegraphics[width=7cm]{graphs/BesselY.pdf}
    \captionof{figure}{前几个第二类Bessel函数}
\end{center}
...
\begin{example}{同心球边界条件}{concentric spheres boundary}
    ...
    \begin{equation}
        \begin{aligned}
            &G(\bm x,\bm x')=\\
            &4\pi\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell \frac{Y_{\ell m}^\ast(\theta',\phi')Y_{\ell m}(\theta,\phi)}{(2\ell+1)\bigfkh{1-(a/b)^{2\ell+1}}}\biggkh{r_<^\ell-\frac{a^{2\ell+1}}{r_<^{\ell+1}}}\biggkh{\frac1{r_>^{\ell+1}}-\frac{r_>^\ell}{b^{2\ell+1}}}.
        \end{aligned}
    \end{equation}
    \tcblower
