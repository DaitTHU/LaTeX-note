...
\begin{example}{接地导体球的镜像电荷}{method of images, grounded sphere conductor}
    ...
    \begin{equation}
        \Phi(\bm r)=\frac q{4\pi\varepsilon_0}\biggkh{\frac1{|\bm r-\bm d|}-\frac ad\frac1{|\bm r-\bm d'|}},\quad\bm d'=\Bigkh{\frac ad}^2\bm d.
    \end{equation}
    \tcblower
...
\begin{equation}
    ...
    G(\bm r,\bm r')=\frac1{|\bm r-\bm r'|}-\frac a{r'}\frac1{|\bm r-\bm r''|},\quad\bm r''=\Bigkh{\frac{a}{r'}}^2\bm r'.
\end{equation}
...
\begin{example}{点电荷电势}{point charge potential}
    ...
    \begin{equation}
        \label{eqn:1/|r-r'|}
        \frac1{|\bm r-\bm r'|}=\frac1{r_>}\sum_{\ell=0}^\infty\biggkh{\frac{r_<}{r_>}}^\ell P_\ell(\cos\gamma).
    \end{equation}
    其中$r_<:=\min(x,x'),\enspace r_>:=\max(x,x')$，$\gamma$是$\bm r,\bm r'$夹角。
\end{example}
...
\begin{equation}
    \label{eqn:1/|r-r'|=YY}
    \frac1{|\bm r-\bm r'|}=\frac{4\pi}{r_>}\sum_{\ell=0}^\infty\biggfkh{\frac1{2\ell+1}\biggkh{\frac{r_<}{r_>}}^\ell\cdot\sum_{m=-\ell}^\ell Y^\ast_{\ell m}(\theta',\phi')Y_{\ell m}(\theta,\phi)}.
\end{equation}
...
事实上，根据我们所介绍的，我们可以简单地得到球坐标下Green函数的展开式。由式\eqref{eqn:1/|r-r'|=YY}，
\[
    \frac1{|\bm r-\bm r'|}=4\pi\sum_{\ell=0}^\infty\biggfkh{\frac1{2\ell+1}\frac{r_<^\ell}{r_>^{\ell+1}}\cdot\sum_{m=-\ell}^\ell Y^\ast_{\ell m}(\theta',\phi')Y_{\ell m}(\theta,\phi)}.
\]
我们希望得到适用于在$r = a$处具有球面边界的外部问题的Green函数的类似展开。由式\eqref{eqn:sphere Green function}易得：
\[
    G(\bm r,\bm r')=\frac1{|\bm r-\bm r'|}-\frac a{r'}\frac1{|\bm r-\bm r''|},\quad\bm r''=\Bigkh{\frac{a}{r'}}^2\bm r'.
\]
对式\eqref{eqn:sphere Green function}中的两项展开\eqref{eqn:1/|r-r'|=YY}，得到
\begin{equation}
    \label{eqn:Green YY}
    G(\bm r,\bm r')=4\pi\sum_{\ell=0}^\infty\biggfkh{\frac1{2\ell+1}\frac1{r_>^{\ell+1}}\biggkh{r_<^\ell-\frac{a^{2\ell+1}}{r_<^{\ell+1}}}\sum_{m=-\ell}^\ell Y_{\ell m}^\ast(\theta',\phi')Y_{\ell m}(\theta,\phi)}.
\end{equation}
...
\begin{example}{同心球边界条件}{concentric spheres boundary}
    ...
    故Green函数的展开
    \begin{equation}
        G(\bm r,\bm r')=4\pi\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell \frac{Y_{\ell m}^\ast(\theta',\phi')Y_{\ell m}(\theta,\phi)}{(2\ell+1)\bigfkh{1-(a/b)^{2\ell+1}}}\biggkh{r_<^\ell-\frac{a^{2\ell+1}}{r_<^{\ell+1}}}\biggkh{\frac1{r_>^{\ell+1}}-\frac{r_>^\ell}{b^{2\ell+1}}}.
    \end{equation}
    \tcblower
...
\]
故$A=4\pi$，我们得到了$1/|\bm r-\bm r'|$的展开 
\begin{equation}
    \frac1{|\bm r-\bm r'|}=\frac1\pi\sum_{m=-\infty}^{+\infty}\int\iti\e{\i m(\phi-\phi')}\e{\i k(z-z')}I_m(k\rho_<)K_m(k\rho_>)\d k.
\end{equation}
...
本征函数是正交的：
\[
    \int_V\psi_m^\ast(\bm r)\psi_n(\bm r)\d V=\vd_{mn}.
\]
...
因此系数为
\[
    a_n(\bm r')=\frac{4\pi}{\lambda_n-\lambda}\int_V\psi^\ast_n(\bm r)\vd(\bm r,\bm r')\d V=4\pi\frac{\psi_n^\ast(\bm r')}{\lambda_n-\lambda}.
\]
因此Green函数的本征函数展开式为
\begin{equation}
    G(\bm r,\bm r')=4\pi\sum_n \frac{\psi_n^\ast(\bm r')\psi_n(\bm r)}{\lambda_n-\lambda}
\end{equation}
对于连续谱，求和用积分代替。
\begin{example}{$1/|\bm r-\bm r'|$的三维Fourier积分表示}{1/|r-r'| Fourier}
    ...
    \[
        G(\bm r,\bm r')=4\pi\int\frac{\psi_{\bm k}^\ast(\bm r')\psi_{\bm k}(\bm r)}{k^2}\d V_k=\frac1{2\pi^2}\int\frac{\e{\i\bm k\cdot(\bm r'-\bm r)}}{k^2}\d V_k.
    \]
\end{example}
