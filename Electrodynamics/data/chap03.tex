...
电荷的局域(localized)分布由电荷密度$\rho(\bm r')$描述，由式\eqref{eqn:Phi-rho}
\[
    \Phi(\bm r)=\frac1{4\pi\varepsilon_0}\int_V\frac{\rho(\bm r')}{|\bm r'-\bm r|}\d V'.
\]
应用式\eqref{eqn:1/|r-r'|=YY}，因为我们对电荷分布之外的电势感兴趣，故$r_<=r',\,r_>=r$
\begin{align}
    \notag
    \Phi(\bm r)&=\frac1{\varepsilon_0}\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell\frac1{2\ell+1}\biggfkh{\int_V\rho(\bm r'){r'^\ell}Y^\ast_{\ell m}(\theta',\phi')\d V'}\frac{Y_{\ell m}(\theta,\phi)}{r^{\ell+1}}\\
...
\begin{align}
    ...
    \mathcal Q_{\alpha\beta}:={}&\int_V(3x_\alpha'x_\beta'-r'^2\vd_{\alpha\beta})\rho(\bm r')\d V'.
\end{align}
则式\eqref{eqn:legendre multipole}可以写成 
\begin{equation}
    \Phi(\bm r)=\frac1{4\pi\varepsilon_0}\biggfkh{\frac qr+\frac{\bm p\cdot\bm r}{r^3}+\frac12\sum_{\alpha,\beta}\frac{\mathcal Q_{\alpha\beta}x_\alpha x_\beta}{r^5}+\cdots}.
\end{equation}
上式也可通过对$1/|\bm r-\bm r'|$在$\bm r'=0$处Taylor展开推导得到。
...
\begin{equation}
    ...
    \bm E(\bm r)=-\nabla\Phi=\frac{3(\bm p\cdot\uvec n)\uvec n-\bm p}{4\pi\varepsilon_0|\bm r-\bm r_0|^3},\quad \uvec n:=\frac{\bm r-\bm r_0}{|\bm r-\bm r_0|}.
    \tag{$\star$}
\end{equation}
式\eqref{eqn:edipole-E without delta}并不严格，事实上在含$\bm r_0$处积分会产生额外的项，应进行修正：
\begin{equation}
    \label{eqn:edipole-E with delta}
    \bm E(\bm r)=\frac1{4\pi\varepsilon_0}\biggfkh{\frac{3(\bm p\cdot\uvec n)\uvec n-\bm p}{|\bm r-\bm r_0|^3}-\frac{4\pi}3\bm p\vd(\bm r-\bm r_0)}.
\end{equation}
...
两个电偶极子之间的互能
\begin{equation}
    W_{12}=\frac{\bm p_1\cdot\bm p_2-3(\uvec n\cdot\bm p_1)(\uvec n\cdot\bm p_2)}{4\pi\varepsilon_0|\bm r_1-\bm r_2|^3},\quad\uvec n:=\frac{\bm r_1-\bm r_2}{|\bm r_1-\bm r_2|}.
\end{equation}
...
\begin{align*}
    \D\Phi(\bm r,\bm r')&=\frac1{4\pi\varepsilon_0}\biggfkh{\frac{\rho(\bm r')}{|\bm r-\bm r'|}\D V+\frac{\bm P(\bm r')\cdot(\bm r-\bm r')}{|\bm r-\bm r'|^3}\D V}\\
    &=\frac1{4\pi\varepsilon_0}\biggfkh{\frac{\rho(\bm r')}{|\bm r-\bm r'|}+\bm P(\bm r')\cdot\nabla'\biggkh{\frac1{|\bm r-\bm r'|}}}\D V
\end{align*}
变成积分并应用分部积分得到
\begin{equation}
    \Phi(\bm r)=\frac1{4\pi\varepsilon_0}\int_V\frac{\rho(\bm r')-\div\bm P(\bm r')}{|\bm r-\bm r'|}\d V'.
\end{equation}
