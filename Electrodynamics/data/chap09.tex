...
%三维一般波动方程
%\lapla u-\frac1{a^2}\pv[2]ut=-f(\bm r,t),
%中的$a$是波速，依惯性系而不同。

\paragraph{Michelson-Morley实验}

人们最开始设想光需要在一种称为以太(ether)的介质中传播，光速是相对于以太的速度。
为了验证以太是否存在以及光速是否是常数，1887年Michelson和Morley设计了如下所示的光学干涉装置：
\begin{center}
	\includegraphics[page=\pagenumref{tikz:Michelson-Morley experiment}]{figures/tikz/layouts.pdf}
	\captionof{figure}{Michelson-Morley光学干涉仪的实验装置图}
	\label{fig:Michelson-Morley}
\end{center}
图中最左侧相干光源发出的光在经过分束器(beam splitter, BS)中心$O$后，其中一部分光被透射后抵达镜子M$_1$然后被反射回$O$点，另一部分光被反射后抵达镜子M$_2$然后被反射回$O$点，并与从M$_1$反射回来的光发生干涉。假设整个装置在以太中以速度$u$向右运动，那么这等价于装置不动，而以太风以速度$u$向左吹来。
...
然而，Michelson-Morley的实验结果发现：不论装置的运动状态和方位如何，光学干涉条纹并未发生任何改变。
这也就从实验上证明了以太并不存在，光速恒为常数$c$！

为了解释这个实验结果，Einstein提出了狭义相对论(special relativity)。
...
相对论动量
\[
    \bm p=m\bm\eta=m\dv{\bm r}\tau=\gamma_um\dv{\bm r}t=\gamma_um\bm u.
\]
有关系
\begin{equation}
    E^2=p^2c^2+m^2c^4.
...
\begin{example}{Compton散射}{Compton scattering}
    ...
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:Compton scattering}]{figures/tikz/layouts.pdf}
...
\begin{example}{导线的磁场力}{}
    ...
    \begin{center}
        \includegraphics[page=\pagenumref{tikz:q in lab frame}]{figures/tikz/layouts.pdf}
        \includegraphics[page=\pagenumref{tikz:q in q frame}]{figures/tikz/layouts.pdf}
