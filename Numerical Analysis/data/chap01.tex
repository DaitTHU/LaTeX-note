...
\begin{remark}
    不严谨地说，完备性要求：所有在$M$中收敛的序列都会收敛到$M$中。%即$M$中没有“缺失点”。
\end{remark}
...
\begin{corollary}
    显然，赋范空间也是度量空间，只需定义$d(a,b):=\norm{a-b}$即可。
\end{corollary}
\begin{remark}
    赋范空间不一定是完备的，完备的赋范空间称为Banach空间。
\end{remark}
...
\begin{example}
    ...
    \begin{proof}
        记$k=\arg\max_i\abs{x_i}$，对下式两边开$p$次方并令$p\to\infty$即得
        \[
    ...
    \begin{center}
        \includegraphics[width=0.7\linewidth]{figures/Lpspace.pdf}
...
\begin{example}
    ...
    \begin{equation}
        ...
        % \end{cases}
    \end{equation}
    当$1<p<\infty$时，$\norm\cdot_p$是不完备的：连续函数的Cauchy序列可以按$p$ - 范数收敛于一个不连续的函数。
...
\begin{definition}
    {算子范数}{operate norm}
    给定向量范数$\norm\cdot$，定义矩阵的算子范数(operate norm)
    \begin{equation}
...
\begin{remark}
    注意$N(I)\equiv 1$。故Frobenius范数不是算子范数。
    \footnote{不考虑$n=1$的平凡情况。}
\end{remark}

\begin{theorem}
    {}{}
    $N(\cdot)$是一个矩阵范数，并与向量范数$\norm\cdot$相容。 
\end{theorem}
...
\begin{remark}
    对于一般的矩阵$A\in\FF^{m\times n}$，有$\sigma(A\dg A)\setminus\{0\}=\sigma(AA\dg)\setminus\{0\}$，即非0特征值相同，称为$A$的奇异值(sigular value)。
\end{remark}
