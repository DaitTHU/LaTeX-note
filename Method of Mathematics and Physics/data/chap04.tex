...
\begin{example}{$\delta$函数与Laplace算子}{}
	\begin{equation}
		\delta(x_1,\ldots,x_n)=\begin{cases}
			\frac1{2\pi}\lapla\ln r,&n=2\\
			-\frac1{(n-2)S_{n}}\lapla\frac1{r^{n-2}},&n\geqslant 3
		\end{cases}
	\end{equation}
...
\begin{example}{3维Helmholtz方程}{}
	...
	\[
		\int_{B_r}\lapla U\d V=\int_{B_r}\delta(x,y,z)\d V=1.
	\]
	由Green公式
	\[
		\int_{B_r}\lapla U\d V=\oint_{\p B_r}\pv Un\d S=-\frac A{r^2}\cdot 4\pi r^2=1,\implies U(r)=-\frac1{4\pi r}.
	\]
	...
	由积分条件
	\[
		\int_{B_r}\lapla U-k^2U\d V=\oint_{\p B_r}\pv Un\d S-k^2\int_{B_r}U\d V=1,
	\]
	且
	\begin{align*}
...
\begin{theorem}{Green公式}{Green Formula}
	...
	\begin{gather}
		...
		\int_\Omega v\lapla u\d V=\oint_{\p\Omega}v\pv u{\bm n}\d S-\int_\Omega\nabla v\cdot\nabla u\d V;\\
		\label{Green's 2nd formula}
		\int_\Omega(u\lapla v-v\lapla u)\d V=\oint_{\p\Omega}\biggkh{u\pv v{\bm n}-v\pv u{\bm n}}\d S.
	\end{gather}
	%其中$\p u/\p\bm n=\nabla u\cdot\bm n$。
\end{theorem}
\begin{proof}
...
\begin{align}
	\begin{cases}
		\lapla u=-f(M),&M\in V\subseteq\RR^3\\
		\edg u_{\p V}=\varphi(M),
	\end{cases}
\end{align}
...
\begin{method}{镜像法}{}
	分解$G=U+g$，$U$满足$\lapla U=-\vd(M-N)$，可取
	\begin{align*}
		U=\begin{cases}
...
\begin{example}{镜像法应用·球域}{}
	...
	\begin{gather*}
		\begin{cases}
			\lapla G(M;N)=-\vd(M-N),&0\leqslant r<R\\
			\edg G_{r=R}=0
		\end{cases}\\
...
\begin{example}{Fourier法应用}{}	
	...
	\begin{align*}
		\begin{cases}
			\lapla G(x,y;\xi,\eta)=-\vd(x-\xi,y-\eta),\quad x,\xi\in[0,L];\;y,\eta\in[0,M]\\
...
\begin{align}
	\begin{cases}
		\lapla u=-f(M),&M\in V\subseteq\RR^n\\
...
\begin{align*}
	\begin{cases}
		\lapla G=-\vd(M-N),&M,N\in V\subseteq\RR^n\\
